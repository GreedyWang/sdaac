...
function sub(flag,prId) {
	...
	if(!document.getElementById('k').checked && document.getElementById('costCenter').value=='-1'){
		alert("成本中心不能为空!");
		return false;
	}
...
//			});	
	...
	prFormListManager.updateBuycontext(delIds,objArray,document.getElementById('uidd').value,function(data) {
	...
	prFormListManager.doSelectAllCostcentre2(area,function(data) {
				...
				if (data != null) {
					for (var i = 0; i < data.length; i++) {
						...
						opt.text = data[i].costCenterNameEnglish
						opt.value = data[i].id
...
var cm = new Ext.grid.ColumnModel([{
	...
	gridstore = new Ext.data.JsonStore({
				...
				fields : [{
						...
						'glAccount', 'orderNo', 'buyerId','id','remark','band']
			})
...
//Change the project id-> show AR_NO
function changeARNO(id){
		prFormListManager.doSelectProject(id,function(data) {
				alert(data.id);
				document.getElementById('arno')
			});
}
