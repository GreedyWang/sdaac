...
this.saveFrom = function () {
				...
				if(window.confirm('确认提交?')){
					...
					pr.form.submit({
								waitTitle : '提示',
								url : 'epor.do?actionType=dorefillin&context='+context2,
								method : 'post',
								// waitMsg:'正在登录验证,请稍候...',
								timeout : 10000,
								// progress:true,
...
function loadData(){
	...
	var fpid = document.getElementById('fpid').value;
	var processInstanceId = document.getElementById('processInstanceId').value;
	var fmanagerUid = document.getElementById('fmanagerUid').value;
	var fprCostCenterOwner = document.getElementById('fprCostCenterOwner').value;
	var fbuyerNetid = document.getElementById('fbuyerNetid').value;
    ...
    Ext.getCmp('arnolist').setValue(farno);	               
	Ext.getCmp('arno').setValue(farno);	
	Ext.getCmp('remark').setValue(fremark);
	Ext.getCmp('prformid').setValue(fpid);
	Ext.getCmp('processInstanceId').setValue(processInstanceId);
	//alert(fcapitalcategroy);
	Ext.getCmp('pm').setValue(fmanagerUid);
	Ext.getCmp('ccowner').setValue(fprCostCenterOwner);
	Ext.getCmp('capitalItem').setValue(fcapitalcategroy);
	
	Ext.getCmp('buyerNetid').setValue(fbuyerNetid);
}
